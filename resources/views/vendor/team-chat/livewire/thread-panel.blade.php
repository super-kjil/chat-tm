<div class="flex h-full flex-col" wire:poll.{{ config('team-chat.polling.messages', 3) }}s>
    {{-- Header --}}
    <div class="flex items-center justify-between px-3 py-2 sm:px-4 sm:py-3 border-b border-gray-200 dark:border-gray-700">
        <h3 class="font-semibold text-gray-900 dark:text-white">{{ __('team-chat::messages.thread') }}</h3>
        <button
            wire:click="$dispatch('close-thread')"
            class="flex items-center gap-1 rounded-md px-2 py-1.5 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"
            title="Close thread"
        >
            {{-- On mobile show label + icon for easier tapping --}}
            <span class="sm:hidden text-xs font-medium">Close</span>
    ...
    <div class="flex-1 overflow-y-auto p-4 space-y-1">
        @foreach($this->replies as $reply)
            @php $isOwner = $reply->user_id === auth()->id(); @endphp
            <div 
                wire:key="reply-{{ $reply->id }}"
                @class([
                    'flex gap-3 rounded-lg px-2 py-1.5 hover:bg-gray-50 dark:hover:bg-gray-800/50',
                    'flex-row-reverse text-right' => $isOwner,
                ])
            >
