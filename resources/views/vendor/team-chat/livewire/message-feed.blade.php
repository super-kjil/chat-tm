<div class="h-full overflow-y-auto p-4 space-y-1" id="message-feed" wire:poll.{{ config('team-chat.polling.messages', 3) }}s>
    ...
    @else
        @foreach($this->messages as $message)
            @php $isOwner = $message->user_id === auth()->id(); @endphp
            <div 
                wire:key="message-{{ $message->id }}"
                @class([
                    'group flex gap-3 rounded-lg px-2 py-1.5 hover:bg-gray-50 dark:hover:bg-gray-800/50',
            ...
            >
                ...
                <div class="min-w-0 flex-1">
                    ...
                    @if($emojiPickerMessageId === $message->id)
                        <div @class([
                            'mt-1 flex flex-wrap gap-1 rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 p-2 shadow-sm',
                            'justify-end' => $isOwner,
                        ])>
                            @foreach(['👍', '👎', '😄', '🎉', '❤️', '🚀', '👀', '🤔'] as $emoji)
                                <button
                    ...
                    @if($message->replies_count > 0)
                        <button
                            wire:click="openThread({{ $message->id }})"
                            @class([
                                'mt-1 flex items-center gap-1 text-xs text-primary-600 dark:text-primary-400 hover:underline',
                                'ml-auto' => $isOwner,
                            ])
                        >
